...
class Electronics : public Item {
    ...
    public:
        ...
        : name(move(other.name)), warranty(move(other.warranty)), price(other.price) {}

        Electronics& operator=(Electronics&& other) noexcept {
        if (this != &other) {
            name = move(other.name);
            warranty = move(other.warranty);
            price = other.price;
            other.price = 0.0f;  
        }
        return *this;
...
class Clothing : public Item {
    ...
    public:
        ...
        : type(move(other.type)), size(move(other.size)), price(other.price) {}

        Clothing& operator=(Clothing other) noexcept {
        if (this != &other) {
...
class Book : public Item {
    ...
    public:
        ...
        : name(move(other.name)), genre(move(other.genre)), price(other.price) {}

        Book& operator=(Book&& other) noexcept {
        if (this != &other) {
            name = move(other.name);
            genre = move(other.genre);
            price = other.price;
            other.price = 0.0f;  
        }
        return *this;
...
class Toys : public Item {
    ...
    public:
        ...
        Toys(Toys&& other) noexcept
        : type(move(other.type)), warranty(move(other.warranty)), price(other.price) {}

        Toys& operator=(Toys&& other) noexcept {
        if (this != &other) {
            type = move(other.type);
            warranty = move(other.warranty);
            price = other.price;
...
unique_ptr<Item> restore(ifstream& in) {
    string type;
    in >> type;
    ...
    if (type == "Grocery") {
        ...
        return make_unique<Grocery>(name, expiryDate, price);
    }
    else if (type == "Electronics") {
        string name, warranty;
        float price;
        in >> name >> price >> warranty;
        return make_unique<Electronics>(name, warranty, price);
    }
    else if (type == "Clothing") {
        string type, size;
        float price;
        in >> type >> price >> size;
        return make_unique<Clothing>(type, size, price);
    }
    else if (type == "Book") {
        string name, genre;
        float price;
        in >> name >> price >> genre;
        return  make_unique<Book>(name, genre, price);
    }
    else if (type == "Toys") {
        string type, warranty;
        float price;
        in >> type >> price >> warranty;
        return make_unique<Toys>(type, warranty, price);
    }
    
    return nullptr;
}

void save(const vector<unique_ptr<Item>>& items, const string& filename) {
    ...
    // Write each item to the file using its persist method
    for (const auto& item : items) {
        item->persist(out); // This will call the appropriate persist method based on the object's type
    }
